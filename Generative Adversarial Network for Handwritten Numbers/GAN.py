...
class Generator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims):
        ...
        self.fc0 = nn.Sequential(nn.Linear(input_dims, 256), nn.LeakyReLU(0.2))  # or 0.3
        # output hidden layer
        self.fc1 = nn.Sequential(nn.Linear(256, 512), nn.LeakyReLU(0.2))  # nn.Tanh()
        self.fc2 = nn.Sequential(nn.Linear(512, 1024), nn.LeakyReLU(0.2))
        self.fc3 = nn.Sequential(nn.Linear(1024, 784), nn.Tanh())
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims=1):
        ...
        self.fc1 = nn.Sequential(nn.Linear(1024, 512), nn.LeakyReLU(0.2), nn.Dropout(0.3))
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    if (num_epoch % epochs == 0):

        genImage = genImage.detach().numpy()
        fig = plt.figure(figsize=(5, 5))
        for i in range(25):
            plt.subplot(5, 5, i + 1)
            plt.imshow(genImage[i].reshape(28, 28), cmap="gray_r")
            plt.xticks([])
            plt.yticks([])
            plt.ioff()
        plt.savefig(path)
